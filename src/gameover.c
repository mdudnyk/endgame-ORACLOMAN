...
char robot111[] = "Press (esc) to Exit    ";

len = strlen(robot111);
    t = robot111;           /* initialize pointer */
    while(len)
    {
        move(20,6);      /* insert same spot */
...
char robot100[] = "Press (R) to Restart  ";

len = strlen(robot100);
    t = robot100;           /* initialize pointer */
    while(len)
    {
        move(19,6);      /* insert same spot */
        insch(*(t+len-1));  /* work backwards */
        refresh();
        napms(10);         /* .1 sec. delay */
        len--;
